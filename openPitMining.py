...
def mycallback(model, where):
    if where == GRB.callback.MESSAGE:
        model.__output.write(model.cbGet(GRB.callback.MSG_STRING))
...
def optimize(cost, value, edges, output=False):
    ...
    log_msg = output.getvalue()

    return [solution, log_msg]
...
if __name__ == '__main__':
    jsdict = {'cost': cost, 'value': value, 'edges': edges}
    jsdict = handleoptimize(jsdict)
    print('Content-Type: application/json\n\n')
    log_msg = jsdict['solution'][1]
    print(log_msg)
